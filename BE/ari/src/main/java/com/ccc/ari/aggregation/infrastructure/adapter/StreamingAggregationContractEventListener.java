...
public class StreamingAggregationContractEventListener {
    ...
    /**
     * 특정 리스너의 RawListenerTracksUpdated 이벤트를 조회합니다.
     *
     * @param listenerId 조회할 리스너의 ID
     * @return 모든 RawListenerTracksUpdated 이벤트 응답 목록
     */
    public List<StreamingAggregationContract.RawListenerTracksUpdatedEventResponse> getRawListenerTracksUpdatedEventsByListenerId(Integer listenerId) {
        logger.info("RawListenerTracksUpdated 이벤트 조회를 시작합니다.");
        try {
            logger.info("스마트 컨트랙트 주소: {}", aggregationContract.getContractAddress());
            ...
            // 두 번째 indexed 파라미터(listenerId)에 대한 필터링
            String encodedListenerId = "0x" + Numeric.toHexStringNoPrefixZeroPadded(BigInteger.valueOf(listenerId), 64);
            filter.addOptionalTopics(encodedListenerId);
            ...
            // 이벤트 응답 반환
            logger.info("RawListenerTracksUpdated 이벤트 조회를 성공적으로 완료했습니다. 총 {}개의 이벤트 응답 반환.",
                    eventResponses.size());
