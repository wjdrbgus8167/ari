...
class SubscriptionPaymentScreen extends ConsumerStatefulWidget {
  ...
  @override
  SubscriptionPaymentScreenState createState() =>
      SubscriptionPaymentScreenState();
}

class SubscriptionPaymentScreenState
  ...
  void initState() {
    ...
    // 구독 타입에 따라 금액 설정
    subscriptionAmount =
        widget.subscriptionType == SubscriptionType.regular ? "1" : "1";
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (mounted) {
        ...
        dev.log(
          "[SubscriptionPaymentScreen] 초기 지갑 상태: 연결=${walletState.isConnected}, 전송 중=${walletState.isTransferring}",
        );
      }
    });
  }
  ...
  void handleSubscription() async {
    ...
    if (chainId == null ||
        !walletService.linkTokenAddresses.containsKey(chainId)) {
      _showSnackBar(
        'LINK 토큰이 지원되지 않는 네트워크입니다. Ethereum, Sepolia 또는 Polygon을 선택해주세요.',
      );
      return;
    }
    ...
    try {
      ...
      final registerResult = await walletService.registerUser(
        ...
        onComplete: (regTxHash, regSuccess, regErrorMessage) {
          dev.log(
            "[구독] 사용자 등록 완료: 성공=$regSuccess, 해시=$regTxHash, 오류=$regErrorMessage",
          );
      ...
      // 3. 구독 시작
      Map<String, dynamic> subscribeResult;
      if (widget.subscriptionType == SubscriptionType.regular) {
        ...
        subscribeResult = await walletService.subscribeRegular(
          ...
          onComplete: (subTxHash, subSuccess, subErrorMessage) {
            dev.log(
              "[구독] 정기 구독 완료: 성공=$subSuccess, 해시=$subTxHash, 오류=$subErrorMessage",
            );
  ...
  Widget build(BuildContext context) {
    ...
    final buttonDisabled =
        isTransferring || isLoading || !isAllAgreed || !isWalletConnected;
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        WalletWidget(defaultAmount: subscriptionAmount),
                        const SizedBox(height: 30),
