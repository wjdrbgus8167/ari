...
class FollowersManagementWidget extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final isChannelInfoLoading =
        channelState.channelInfoStatus == MyChannelStatus.loading;
    ...
    return ElevatedButton(
      ...
      style: ElevatedButton.styleFrom(
        backgroundColor:
            isFollowing ? Colors.grey[800] : AppColors.mediumPurple,
  ...
  Future<void> _handleFollowAction(WidgetRef ref) async {
    ...
    if (isFollowing) {
      // 이미 팔로우 중이면 언팔로우 - 추가된 코드로 memberId 바로 사용
      await channelNotifier.unfollowMember(targetMemberId);
    } else {
      // 언팔로우 중이면 팔로우
