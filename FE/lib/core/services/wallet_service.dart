...
typedef TransactionCallback = void Function(String? txHash, bool success, String? errorMessage);
...
class WalletService extends ChangeNotifier {
  ...
  final Map<String, String> linkTokenAddresses = {
    ...
    'eip155:11155111': '0x779877A7B0D9E8603169DdbD7836e478b4624789', // Sepolia 테스트넷
  ...
  String getCurrentSubscriptionContractAddress() {
    if (_appKitModal == null || _appKitModal!.selectedChain == null) {
      return subscriptionContractAddresses[sepoliaChainId] ?? ''; // 기본값으로 Sepolia 사용
    }
  ...
  void _setupSupportedNetworks() {
    ...
    final supportedEVMChains = ['1', '11155111', '137']; // 이더리움, Sepolia, Polygon
    
    final allEVMChains = ReownAppKitModalNetworks.getAllSupportedNetworks(namespace: 'eip155');
  ...
  Future<Map<String, dynamic>> _callContractFunction({
    ...
    try {
      ...
      final namespace = ReownAppKitModalNetworks.getNamespaceForChainId(rawChainId);
      dev.log("$prefix 네임스페이스: $namespace");
      dev.log("주소 HEX: ${_appKitModal!.session!.getAddress(namespace)!}");
      // 트랜잭션 객체 생성
      final transaction = Transaction(
        from: EthereumAddress.fromHex(_appKitModal!.session!.getAddress(namespace)!),
      );
  ...
  Future<Map<String, dynamic>> approveERC20Token({
    ...
    try {
      ...
      dev.log("[WalletService] 변환된 승인 금액: $amount -> $amountInWei wei (소수점: $tokenDecimals)");
    } catch (parseError) {
